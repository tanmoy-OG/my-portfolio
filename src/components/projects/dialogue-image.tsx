...
const DialogueImage = ({ title, gallery, isPopup, initialIndex = 0 }: Params) => {
    return (
        <div className="fixed flex justify-center top-0 left-0 size-full z-20 py-8 backdrop-blur-lg" onClick={() => isPopup(false)}>
            <Card className="w-11/12 lg:w-5/6 px-4 lg:px-16 py-8 h-fit lg:h-auto my-auto lg:my-0 flex justify-center items-center bg-muted/70 border-foreground/10 shadow-secondary" onClick={(e) => e.stopPropagation()}>
                <Carousel className="w-full flex flex-col gap-4 lg:block" opts={{ startIndex: initialIndex }}>
                    <CarouselContent>
                        {gallery.map((link, id) => (
                            <CarouselItem key={id}>
                                <div className="p-1">
